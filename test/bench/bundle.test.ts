...
describe("benchmark", () => {
  it("bundle size (H3)", async () => {
    ...
    expect(bundle.bytes).toBeLessThanOrEqual(18_400); // <18.4kb (RFC 9457 problem+json support)
  ...
  it("bundle size (H3Core)", async () => {
    ...
    expect(bundle.bytes).toBeLessThanOrEqual(8800); // <8.8kb (RFC 9457 problem+json support)
    expect(bundle.gzipSize).toBeLessThanOrEqual(3_600); // <3.6kb
  });
  ...
  it("bundle size (defineHandler)", async () => {
    ...
    expect(bundle.bytes).toBeLessThanOrEqual(7900); // <7.9kb (RFC 9457 problem+json support)
    expect(bundle.gzipSize).toBeLessThanOrEqual(3_300); // <3.3kb
  });
});
