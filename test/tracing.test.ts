...
import type {
  H3InitPayload,
  H3MountPayload,
  H3RequestHandlerPayload,
} from "../src/index.ts";

type TracingEvent = {
  start?: { data: H3RequestHandlerPayload };
  end?: { data: H3RequestHandlerPayload };
  asyncStart?: { data: H3RequestHandlerPayload };
  asyncEnd?: { data: H3RequestHandlerPayload; result?: any; error?: Error };
  error?: { data: H3RequestHandlerPayload; error: Error };
};
...
describeMatrix("tracing channels", (t, { it, expect }) => {
  ...
  it("tracing channels fire for handlers", async () => {
    ...
    try {
      ...
      expect(firstStart.asyncStart?.data.request).toBeDefined();
  ...
  it("request and event objects are included in tracing payloads", async () => {
    ...
    try {
      ...
      expect(firstEvent.request).toBeDefined();
      expect(firstEvent.event).toBeDefined();
      expect(firstEvent.request.method).toBe("GET");
