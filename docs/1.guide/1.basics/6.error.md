...
> [!NOTE]
> When an error is thrown, any [prepared headers](/guide/basics/response#preparing-response) set via `event.res.headers` are **not** included in the error response. To set headers that should persist on error responses (e.g., CORS headers), use `event.res.errHeaders`. Built-in utilities like `handleCors` automatically set both.
