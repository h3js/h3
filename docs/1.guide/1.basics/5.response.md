...
> [!NOTE]
> If an Error happens, prepared status and headers will be discarded. To set headers that persist on error responses, use `event.res.errHeaders` instead. This is useful for CORS headers that should be present even when an error is thrown.
>
> ```js
> // These headers will be included in error responses
> event.res.errHeaders.set("access-control-allow-origin", "https://example.com");
> ```
