...
A percent-encoded unreserved character (`ALPHA` / `DIGIT` / `-` / `.` / `_` / `~`) is equivalent to the decoded one per [RFC 3986 §6.2.2.2](https://www.rfc-editor.org/rfc/rfc3986#section-6.2.2.2) — every decoding consumer reads `/%61dmin` as `/admin`, while H3's matchers compare them as two different strings. Left alone, that gap lets `/%61dmin` slip past an `/admin` guard and still reach the `/admin` route, or reach a catch-all handler that decodes it downstream. So H3 decodes exactly those escapes, once, before routing. Nothing else is touched:
...
| `/100%25`     | `/100%25`            | `%` is not unreserved                         |
| `/a%20b`      | `/a%20b`             | Space is not unreserved                       |
| `/caf%C3%A9`  | `/caf%C3%A9`         | Non-ASCII is not unreserved                   |
...
Canonicalization happens in the `H3Event` constructor, so it covers every event, including one built by `mockEvent()` or a standalone `handler.fetch()`. Set the [`allowNonCanonicalURL`](/guide/api/h3#h3-options) app option to dispatch the raw pathname instead — every pathname-based guard in your app is then responsible for the encoded spellings too.
