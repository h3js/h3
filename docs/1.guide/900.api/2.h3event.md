...
| `/a/%2e%2e/b` | `308` → `/b`               | The URL parser resolves the dot segment first |
...
Without this, `/%61dmin` would slip past an `/admin` guard and still reach the `/admin` route — or, with a catch-all route, reach a handler that decodes it downstream. Set the [`allowNonCanonicalURL`](/guide/api/h3#h3-options) app option to dispatch such requests with the raw pathname instead; every pathname-based guard in your app is then responsible for the encoded spellings too.
