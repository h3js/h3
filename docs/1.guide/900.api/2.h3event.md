...
The decoded set is the escapes whose character survives WHATWG path serialization unchanged, minus `%25`: the [RFC 3986 §2.3](https://www.rfc-editor.org/rfc/rfc3986#section-2.3) unreserved set (`ALPHA` / `DIGIT` / `-` / `.` / `_` / `~`), which is equivalent to its literal per [§6.2.2.2](https://www.rfc-editor.org/rfc/rfc3986#section-6.2.2.2), plus `!`, `'`, `(`, `)`, `*`, `[`, `]` and `|`. Decoding anything else either cannot survive the round trip (`%20`, `%5E`, non-ASCII), would change how many segments the path has (`%2f`, `%5c`), would delete the character outright (`%09`), or would expose a nested escape (`%25`).
