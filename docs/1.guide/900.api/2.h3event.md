...
Register a callback that runs once the event is fully over: the response body finished streaming, the client disconnected, or the body errored. This is the portable place to release per-request resources (timers, upstream connections, file handles) on every runtime — not just Node.js.
...
The callback receives `undefined` when the response completed normally, or the cancel/abort reason otherwise:

```js
event.onDispose((reason) => {
  if (reason === undefined) {
    // response fully delivered by h3
  } else {
    // client disconnected or body errored
  }
});
```
...
- Fires exactly once, after the global `onResponse` hook, on every termination path (normal end, client cancel, body error).
- Callbacks run in registration order. Sync throws and async rejections are absorbed (reported via `console.error` unless the app is configured with `silent`), and pending async callbacks are passed to `waitUntil`.
- Registering after disposal invokes the callback immediately. Register during request handling (handler, middleware, or `onResponse`) to be guaranteed to observe the end of the event.
- Opt-in cost: routes that never call `onDispose` pay nothing.

> [!NOTE]
> `onDispose` signals _"h3 is done with this event"_, not _"the client received the response"_. For non-streaming bodies on non-Node.js runtimes it fires when the response is handed to the runtime. To react to a client disconnect _while still producing_ the response (for example to abort an upstream fetch), use `event.req.signal` instead.
