...
- `allowNonCanonicalURL`: When enabled, requests whose URL path percent-encodes an unreserved character (e.g. `/%61dmin`) are dispatched as-is instead of being answered with a `308` redirect to the canonical form (the default). See [pathname encoding](/guide/api/h3event#pathname-encoding).
