...
- `allowNonCanonicalURL`: When enabled, a URL path that percent-encodes an unreserved character (e.g. `/%61dmin`) is dispatched with the raw pathname, instead of being canonicalized to `/admin` before routing (the default). See [pathname encoding](/guide/api/h3event#pathname-encoding).
