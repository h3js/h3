...
- `canonicalURL`: How to handle a request whose URL path percent-encodes an unreserved character (e.g. `/%61dmin`): `"redirect"` (default) answers with a `308` to the canonical form, `"rewrite"` canonicalizes `event.url` and dispatches, `false` dispatches the raw pathname. See [pathname encoding](/guide/api/h3event#pathname-encoding).
