...
- ✅ Cross-environment compatibility (shared frontend and backend code)
- ✅ Easier server API testing

## Srvx: Universal Server API

A major challenge is that Node.js lacks native support for Web-standard HTTP servers. Adapters are needed to bridge `node.http.IncomingMessage` to `Request`, and to handle `Response` via `node.http.ServerResponse`. Existing adapters did not fully meet H3's requirements for modularity, performance, and compatibility. We have implemented a [compatibility layer](https://srvx.h3.dev/guide/node) that synchronizes interfaces and achieves up to **90%** of `node:http` performance—almost the best possible performances today comparing to alternatives. see [benchmarks](https://github.com/h3js/srvx/tree/main/test/bench-node)).
...
We created [💥 srvx](https://srvx.h3.dev) out of H3 development branch: An open, robust abstraction layer aiming to unify JavaScript server APIs:
...
Adopting Web APIs and leveraging [💥 srvx](https://srvx.h3.dev) simplified H3’s scope significantly.

- ✅ A Web-standard server optimized for performance.
- ✅ Simple responses and error handling (return or throw directly).
- ✅ Fast routing via the tiny router ([🌳 rou3](https://github.com/h3js/rou3)).
- ✅ Middleware for request, response, and error interception.
...
H3 now offers an ergonomic, composable way to chain middleware inspired by [Hono middleware](https://hono.dev/docs/guides/middleware) 💛:
...
| ---------------------- | ------- | -------------------- |
| Web API latency (node) | 36 µs   | 7 µs (5x faster)     |
| Web API latency (bun)  | 3.3 µs  | 27.74 µs (8x faster) |
| Web API latency (deno) | 1.3 ms  | 8 µs (160x faster!)  |
| App bundle (min)       | 101 kB  | 9,1 kB (91% smaller) |
| App bundle (min+gzip)  | 39.6 kB | 3.6 kB (90% smaller) |

> [!NOTE]
> The benchmarks above compare the H3 core with the **Web Standard** target and do not include adapters. The effect on real-world performance may be negligible in larger applications. See the [benchmarks source](https://github.com/h3js/h3/tree/main/test/bench) for details.
