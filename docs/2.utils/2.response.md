...
A pathname starting with more than one separator (`//private/x`, `/\\private/x`) is **not served** (404, or falls through when `fallthrough` is set): it dispatches to a catch-all route but misses a narrower `use("/private/**")` guard, and the only `id` `serveStatic` could build from it collapses that leading run, re-spelling it into a path the guard would have caught. Assets are reachable under their canonical spelling only.
