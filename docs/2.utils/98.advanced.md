...
```ts import { requireBasicAuth } from "h3"; import { defineEventHandler } from "h3";  export default defineEventHandler(async (event) => {  await requireBasicAuth(event, { username: "test", password: "test" });  return `Hello, ${event.context.basicAuth.username}!`; });
