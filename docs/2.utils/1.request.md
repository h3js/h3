...
### `getResponseAcceptQuery(event)`

Read the `Accept-Query` media types previously set on the response via {@link setResponseAcceptQuery}.

The Structured Fields List is parsed back into an array of media type strings with any quoted parameter values unquoted (e.g. `application/sql;charset="UTF-8"` becomes `application/sql;charset=UTF-8`). Returns an empty array if the header is not set.
...
Throws: - `400 Bad Request` if the `Content-Type` header is missing. - `422 Unprocessable Content` if the `Content-Type` header is malformed. - `415 Unsupported Media Type` if the media type is not accepted.
...
### `setResponseAcceptQuery(event, mediaTypes)`
